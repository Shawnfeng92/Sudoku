...
for (i in 0:2) {
  for (j in 0:2) {
    for (k in c(1:8,0)) {
      temp <- rep(2, 729)
      temp[which(
        (as.integer((1:729)/81) > i*3) & (as.integer((1:729)/81) <= (i+1)*3) &
          ((as.integer((1:729)/9)%%9+1) > j*3) & ((as.integer((1:729)/9)%%9+1) <= (j+1)*3) &
          ((1:729)%%9 == k)
      )] <- 1
    }
    temp[which(temp > 1)] <- 0
...
# export constraint matrix to debug
write.csv(const.mat, "try.csv", row.names = FALSE, col.names = FALSE)
